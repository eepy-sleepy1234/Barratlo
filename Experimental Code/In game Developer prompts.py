...
class input_box():
    def __init__(self, x, y ,w , h , text, color, text_color):
        ...
        self.input = False
        self.input_text = ""
        self.font = pygame.font.Font((os.path.join(FONTS_DIR, 'OSD mono.ttf')), int(h))
        self.text_color = text_color
        input_boxes.append(self)
    def draw(self, screen):
        if self.shouldDraw:
                    pygame.draw.rect(screen, self.color, self.rect)
                    display_text = self.input_text if self.input_text else self.text
...
input_dev = input_box(0, 0, WIDTH, HEIGHT/25, "type help for list of commands", black, white,)
    ...
    def check_dev(self):
        ...
        if self == DEV_MODE:
            if developer == False:
                ...
                result = input_dev.check_input()
                if result is None or result.strip() == "":
                    developer = False
                    DEV_MODE.toggle = False
                    return
                    
                if result is not None and result == devtoggle:
                    developer = True

                    DEV_MODE.toggle = True
                    input_dev.input = False  
                    input_dev.shouldDraw = False  
                    input_dev.reset_input()
...
def dev_commands():
    ...
    if developer:
        if DEV_MODE.toggle:
            if dev_toggle:
                dev_command = input_dev.check_input()
